...
# ``container`` parks an epic/scaffold whose stories carry the work: like a
# generic human blocker it stays in ``blocked`` (that IS its steady state), but
# when ``kanban.container_parent_gate`` is enabled, the parent gate treats a
# blocked ``container`` parent as satisfied so linked stories can be claimed
# without waiting for the epic (RFC 2026-09-06).
...
def unsatisfied_parents(conn: sqlite3.Connection, task_id: str) -> list[tuple[str, str]]:
    ...
    A ``blocked`` epic container satisfies the gate while
    ``kanban.container_parent_gate`` is enabled, so it is filtered out here
    too: this listing must agree with :func:`_parents_satisfied`, which is the
    function the claim and completion gates actually consult.
    """
    rows = conn.execute(
        "SELECT p.id, p.status, p.block_kind FROM task_links l "
        "JOIN tasks p ON p.id = l.parent_id "
        "WHERE l.child_id = ? AND p.status NOT IN ('done', 'archived') "
        "ORDER BY p.id", (task_id,),
    ).fetchall()
    if _container_parent_gate_enabled():
        rows = [
            r for r in rows
            if not (
                r["status"] == "blocked"
                and (
                    r["block_kind"] == "container"
                    or _is_container_parent(conn, r["id"])
                )
            )
        ]
...
def block_task(
    ...
    toward the loop breaker so a forever-flaky task escalates. ``container``
    parks an epic/scaffold whose stories carry the work: it stays in
    ``blocked`` (that IS its steady state — not auto-resumed, not
    dependency-routed) and participates in the loop breaker like a generic
    block; with ``kanban.container_parent_gate`` enabled the parent gate
    treats it as satisfied so linked stories can be claimed and completed
    without waiting for the epic. True on any transition.
